'use client'

import { useState } from 'react'
import { useRouter } from 'next/navigation'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import { Textarea } from '@/components/ui/textarea'
import { ArrowLeft, Save, Loader2, Folder } from 'lucide-react'
import Link from 'next/link'

interface CriarPastaFormProps {
  plano: string
}

export function CriarPastaForm({ plano }: CriarPastaFormProps) {
  const router = useRouter()
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  // Limites por plano
  const LIMITES = {
    free: { max_pastas: 0 },
    basic: { max_pastas: 5 },
    premium: { max_pastas: 999999 },
  }

  const limite = LIMITES[plano as keyof typeof LIMITES] || LIMITES.free

  // Opções de cores predefinidas (baseadas no TEC Concursos)
  const CORES_PREDEFINIDAS = [
    { nome: 'Verde (Padrão)', hex: '#8fbc8f' },
    { nome: 'Azul', hex: '#4a90e2' },
    { nome: 'Roxo', hex: '#9b59b6' },
    { nome: 'Laranja', hex: '#f39c12' },
    { nome: 'Vermelho', hex: '#e74c3c' },
    { nome: 'Rosa', hex: '#e91e63' },
    { nome: 'Ciano', hex: '#00bcd4' },
    { nome: 'Amarelo', hex: '#ffd700' },
  ]

  // Form state
  const [formData, setFormData] = useState({
    nome: '',
    descricao: '',
    cor: '#8fbc8f',
    icone: 'folder',
  })

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setLoading(true)
    setError(null)

    try {
      // Validar plano
      if (plano === 'free') {
        throw new Error('Recurso não disponível no plano FREE. Faça upgrade para criar pastas.')
      }

      // Preparar dados
      const payload = {
        nome: formData.nome,
        descricao: formData.descricao || undefined,
        cor: formData.cor,
        icone: formData.icone,
        ordem: 0,
      }

      const response = await fetch('/api/pastas', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      })

      const data = await response.json()

      if (!response.ok) {
        throw new Error(data.error || 'Erro ao criar pasta')
      }

      // Sucesso - redirecionar para a lista de cadernos
      router.push('/dashboard/cadernos')
      router.refresh()
    } catch (err: any) {
      setError(err.message)
      setLoading(false)
    }
  }

  // Se FREE, mostrar paywall
  if (plano === 'free') {
    return (
      <div>
        {/* Header */}
        <div className="mb-8">
          <Link
            href="/dashboard/cadernos"
            className="inline-flex items-center text-gray-400 hover:text-white transition-colors mb-4"
          >
            <ArrowLeft className="w-4 h-4 mr-2" />
            Voltar para Meus Cadernos
          </Link>
          <h1 className="text-3xl font-bold text-white">Criar Nova Pasta</h1>
        </div>

        {/* Paywall */}
        <div className="max-w-2xl mx-auto">
          <div className="rounded-xl border border-gray-800 bg-gray-900/50 p-8 text-center">
            <div className="mx-auto w-16 h-16 rounded-full bg-yellow-500/10 flex items-center justify-center mb-4">
              <Folder className="w-8 h-8 text-yellow-500" />
            </div>
            <h2 className="text-2xl font-bold text-white mb-2">
              Organize seus cadernos em pastas
            </h2>
            <p className="text-gray-400 mb-6">
              O sistema de pastas está disponível apenas nos planos BÁSICO e PREMIUM.
            </p>
            <div className="space-y-4">
              <div className="text-sm text-gray-400">
                <p className="mb-2">Com pastas você pode:</p>
                <ul className="list-disc list-inside text-left max-w-md mx-auto">
                  <li>Organizar cadernos por concurso (PM-SP, PRF, PF, etc.)</li>
                  <li>Criar estruturas hierárquicas com subpastas</li>
                  <li>Personalizar cores e ícones</li>
                  <li>Visualizar estatísticas por pasta</li>
                </ul>
              </div>
              <Link href="/dashboard/assinatura">
                <Button className="bg-[#8fbc8f] hover:bg-[#7aa87a] text-gray-900 font-medium">
                  Fazer Upgrade do Plano
                </Button>
              </Link>
            </div>
          </div>
        </div>
      </div>
    )
  }

  return (
    <div>
      {/* Header */}
      <div className="mb-8">
        <Link
          href="/dashboard/cadernos"
          className="inline-flex items-center text-gray-400 hover:text-white transition-colors mb-4"
        >
          <ArrowLeft className="w-4 h-4 mr-2" />
          Voltar para Meus Cadernos
        </Link>
        <h1 className="text-3xl font-bold text-white">Criar Nova Pasta</h1>
        <p className="text-gray-400 mt-2">
          Organize seus cadernos em pastas personalizadas
        </p>
      </div>

      {/* Formulário */}
      <form onSubmit={handleSubmit} className="max-w-2xl space-y-6">
        {/* Erro Geral */}
        {error && (
          <div className="rounded-lg border border-red-500/50 bg-red-500/10 p-4">
            <p className="text-sm text-red-500">{error}</p>
          </div>
        )}

        {/* Nome */}
        <div>
          <label htmlFor="nome" className="block text-sm font-medium text-gray-300 mb-2">
            Nome da Pasta <span className="text-red-500">*</span>
          </label>
          <Input
            id="nome"
            type="text"
            placeholder="Ex: Concurso PM-SP 2025"
            value={formData.nome}
            onChange={(e) => setFormData({ ...formData, nome: e.target.value })}
            required
            minLength={2}
            maxLength={200}
            className="bg-gray-900 border-gray-800 text-white"
          />
          <p className="text-xs text-gray-500 mt-1">
            Mínimo 2 caracteres, máximo 200
          </p>
        </div>

        {/* Descrição */}
        <div>
          <label htmlFor="descricao" className="block text-sm font-medium text-gray-300 mb-2">
            Descrição (opcional)
          </label>
          <Textarea
            id="descricao"
            placeholder="Ex: Pasta com todos os cadernos para o concurso da Polícia Militar de São Paulo"
            value={formData.descricao}
            onChange={(e) => setFormData({ ...formData, descricao: e.target.value })}
            rows={3}
            className="bg-gray-900 border-gray-800 text-white resize-none"
          />
        </div>

        {/* Cor */}
        <div>
          <label className="block text-sm font-medium text-gray-300 mb-2">
            Cor da Pasta
          </label>
          <div className="grid grid-cols-4 md:grid-cols-8 gap-3">
            {CORES_PREDEFINIDAS.map((cor) => (
              <button
                key={cor.hex}
                type="button"
                onClick={() => setFormData({ ...formData, cor: cor.hex })}
                className={`group relative rounded-lg p-3 border-2 transition-all ${
                  formData.cor === cor.hex
                    ? 'border-white'
                    : 'border-gray-800 hover:border-gray-700'
                }`}
                title={cor.nome}
              >
                <div
                  className="w-full h-8 rounded"
                  style={{ backgroundColor: cor.hex }}
                />
                {formData.cor === cor.hex && (
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="w-6 h-6 rounded-full bg-white flex items-center justify-center">
                      <div className="w-3 h-3 rounded-full bg-gray-900" />
                    </div>
                  </div>
                )}
              </button>
            ))}
          </div>
          <p className="text-xs text-gray-500 mt-2">
            Cor selecionada: <span style={{ color: formData.cor }}>{formData.cor}</span>
          </p>
        </div>

        {/* Botões */}
        <div className="flex items-center gap-4 pt-4">
          <Button
            type="submit"
            disabled={loading || !formData.nome}
            className="bg-[#8fbc8f] hover:bg-[#7aa87a] text-gray-900 font-medium"
          >
            {loading ? (
              <>
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                Criando...
              </>
            ) : (
              <>
                <Save className="mr-2 h-4 w-4" />
                Criar Pasta
              </>
            )}
          </Button>
          <Link href="/dashboard/cadernos">
            <Button
              type="button"
              variant="outline"
              disabled={loading}
              className="border-gray-800 text-gray-300 hover:bg-gray-900"
            >
              Cancelar
            </Button>
          </Link>
        </div>

        {/* Informação de Limite */}
        <div className="rounded-lg border border-gray-800 bg-gray-900/30 p-4">
          <p className="text-sm text-gray-400">
            <span className="font-medium text-white">Plano {plano.toUpperCase()}:</span>{' '}
            Você pode criar até {limite.max_pastas === 999999 ? 'ilimitadas' : limite.max_pastas} pasta(s).
          </p>
        </div>
      </form>
    </div>
  )
}
